...
export async function initFall() {
  ...
  /* A phone carries half the field and no hairline. Both are texture: sixty
     blurred layers and a second full screen canvas cost a handset a frame
     every frame, and at this size nobody is counting the cards behind him.
     Read once, because a handset does not change width mid scroll. */
  const pocket = innerWidth < 760;

  drawStars(starCanvas);
  const cards = buildField(field, front, pocket);
  ...
  const rim = pocket ? null : $('#voicesRim');
  const rctx = rim?.getContext('2d');
  if (pocket) $('#voicesRim')?.remove();
  ...
  function measure() {
    ...
    silFrame = -1;
    layout(cards, vw, vh, drawW, pocket);
  }
  ...
  whileVisible(section, (t) => {
    ...
    stream(cards, flow, span, vh, reach, s, !pocket);
...
function buildField(back, front, pocket) {
  /* every featured row, and on a phone only every other one of the rest */
  const rows = pocket
    ? COMMENTS.filter((c, i) => c.hi || i % 2 === 0)
    : COMMENTS;
  let rank = 0;
  return rows.map((c, i) => {
    const node = card(c, i);
...
function layout(cards, vw, vh, charW, pocket) {
  const narrow = pocket || vw < 760;
...
function stream(cards, flow, span, vh, run, s, sway) {
  for (const card of cards) {
    ...
    if (!shown) continue;
    /* The sway is the expensive part: a rotation that changes every frame
       makes a blurred card rasterise again every frame. Held still, the card
       only moves, which the compositor does for nothing. */
    const rot = sway ? card.rot + Math.sin(s * 0.32 + card.seed) * (card.hi ? 0.8 : 2.6) : card.rot;
