...
/* what each سراج says when the ring comes round to them */
const LINES = {
  misbah: ['شوفوا الإضاءة في هاللقطة', 'هذي من الحلقة الجاية، لا تسربونها'],
  siraj: ['مين جاهز لتحدي الأسبوع؟', 'ارسموني بلون جديد!'],
  fanous: ['هههههه لا مستحيل', 'عندي نظرية عن النهاية'],
  qandeel: ['صوتوا قبل الجمعة', 'أنا أرسم الخلفية الليلة']
};
...
export function initCommunity() {
  ...
  const run = () => {
    ...
    running = go;
    clearTimeout(timer);
    if (go) timer = setTimeout(turn, 600);
  };
