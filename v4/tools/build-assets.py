...
OUT = os.path.join(ROOT, "v4", "assets")
...
def crop_avatar(im):
    ...
    place. Scan the left strip for pixels that differ from the background and
    keep only the wide rows: the reply thread line under an avatar is a few
    pixels across, the avatar itself is far wider."""
    rgb = im.convert("RGB")
    W = rgb.width
    px = rgb.load()
    bg = px[4, 4]
    x1, y1 = int(W * 0.115), min(rgb.height, int(W * 0.2))
    wide = []
    for y in range(y1):
        xs = [x for x in range(x1) if sum(abs(a - b) for a, b in zip(px[x, y], bg)) > 24]
        if len(xs) > W * 0.012:
            wide.append((y, xs[0], xs[-1]))
    run = wide[:1]
    for row in wide[1:]:
        if row[0] != run[-1][0] + 1:
            break
    ...
    if len(run) > W * 0.03:
        ...
        left, right = min(r[1] for r in run), max(r[2] for r in run)
        cx, cy, d = (left + right) / 2, (top + bottom) / 2, right - left + 1
    else:
        # avatar the same colour as the page (a plain black one in dark mode):
        # fall back to where YouTube on iOS puts it at 1170 px wide
        k = W / 1170
        cx, cy, d = 72 * k, 72 * k, 72 * k
        print(f"         avatar not found, used the default spot. check it")
